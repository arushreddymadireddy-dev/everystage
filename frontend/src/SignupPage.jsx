 ...
 * Everystage — Sign up
 * -------------------------------------------------------------
 * Talks to the auth API in /backend (POST /api/auth/register).
 * On success, behaves exactly like LoginPage: calls onAuthenticated
 * with the new user + accessToken (registering logs you straight in).
 *
 * Usage:
 *   <SignupPage
...
const COPY = {
  student: {
    emphasis: "begins",
    greet: "Start your record",
    sub: "Create your student profile — it'll follow you well past graduation.",
    accent: "#E8A33D",
  },
  alumni: {
    emphasis: "continues",
    greet: "Rejoin the network",
    sub: "Set up your alumni profile to reconnect with mentors and peers.",
    accent: "#2FB8A6",
  },
};

export default function SignupPage({ onAuthenticated, onSwitchToLogin }) {
  ...
  function validate() {
    ...
    if (!hasMinLength || !hasNumber) next.password = "Password needs 8+ characters and a number.";
  ...
  return (
    <div className="grid min-h-screen grid-cols-1 md:grid-cols-[42%_1fr] bg-[#FDFCF9] text-[#0F1830]">
      {/* ============ LEFT: STAGE PATH ============ */}
      <aside
        className="relative hidden md:flex flex-col justify-between overflow-hidden p-11 text-[#FDFCF9]"
          ...
          <form onSubmit={handleSubmit} noValidate>
            ...
            {password.length > 0 && (
              <motion.div
                ...
                className="mb-4.5 -mt-2.5 flex gap-4 pl-0.5"
              >
                <Requirement met={hasMinLength} label="8+ characters" />
                <Requirement met={hasNumber} label="A number" />
              </motion.div>
            )}
            ...
            <motion.button
              ...
              whileHover={{ y: -1 }}
              whileTap={{ scale: 0.99 }}
              className="relative mt-2 flex w-full items-center justify-center rounded-[14px] py-4 font-bold text-[15px] text-[#FDFCF9] transition-colors"
...
function Requirement({ met, label }) {
  return (
    <div className={`flex items-center gap-1.5 text-[12px] transition-colors ${met ? "text-[#2FB8A6]" : "text-[#8B92A4]"}`}>
      {met ? <Check size={13} strokeWidth={2.5} /> : <X size={13} strokeWidth={2} />}
      {label}
    </div>
  );
}
